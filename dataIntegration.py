...
def select_the_dataset(n):
    ...
    if n == '19':
        ...
        appreciate = pd.read_csv("data/Behance/Behance_appreciate_1M", header=None, sep=' ')
        appreciate.columns = ['user_id', 'item_id', 'unix_timestamp']
        owners = pd.read_csv("data/Behance/Behance_Item_to_Owners", header=None, sep=' ')
        owners.columns = ['item_id', 'owner_id']
        owners_grouped = owners.groupby('item_id')['owner_id'].agg(list).reset_index()
        appreciate_owners = appreciate.merge(owners_grouped, on='item_id', how='left')

        items = []
        with open("data/Behance/Behance_Image_Features.b", 'rb') as f:
            while True:
                item_id_bytes = f.read(8)
                if not item_id_bytes:
                    break
                item_id = int(item_id_bytes.decode('ascii'))
                feature_bytes = f.read(4 * 4096)
                if len(feature_bytes) != 4 * 4096:
                    continue
                feature = struct.unpack('f' * 4096, feature_bytes)
                items.append((item_id, feature))

        Image_Features = pd.DataFrame(items, columns=['item_id', 'image_features'])
        df = appreciate_owners.merge(Image_Features, on='item_id', how='left')
